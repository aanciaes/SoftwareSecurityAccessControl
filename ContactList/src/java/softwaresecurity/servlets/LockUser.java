    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                String user = request.getParameter("lockuser");
                String sqlStr = "SELECT isLocked FROM users where username = '" + user + "';";

                ResultSet rset = stmt.executeQuery(sqlStr); // Send the query to the server

                if (rset.isClosed()) {
                    out.println("No such user in the system<br>");
                } else {
                    if (rset.getBoolean("isLocked")) {
                        out.println("User is already Locked<br>");

                    } else {
                        stmt = conn.createStatement();
                        sqlStr = "update users set isLocked = 1 where username = '" + user + "';";
                        stmt.executeUpdate(sqlStr); // Send the query to the server
                        out.println("User locked<br>");
                    }
                }
